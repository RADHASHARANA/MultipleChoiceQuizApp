...
struct model
{
    ...
    func getOptions(buttonNumber: Int)->String
    {
        return quiz[questionNumber].options[buttonNumber]
    }
    
    mutating func checkAnswer(userOption: UIButton)->Bool
    {
        if(userOption.currentTitle == quiz[questionNumber].answer)
        {
            score+=1
            return true
        }
